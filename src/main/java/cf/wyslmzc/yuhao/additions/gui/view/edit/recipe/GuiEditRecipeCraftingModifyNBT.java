...
import cf.wyslmzc.yuhao.additions.gui.message.GuiMessageBoxNeedsRestart;
...
import cf.wyslmzc.yuhao.additions.gui.view.components.input.GuiComponentIngredientModifierInput;
 ...
 * @since April 2019
 */
public class GuiEditRecipeCraftingModifyNBT extends GuiEdit {
	
	protected Addon addon;
	
    protected boolean isNew;
    protected RecipeAddedCraftingModifyNBT addition;
    protected RecipeAddedCraftingModifyNBT copyFrom;
    ...
    protected GuiComponentListInput<GuiComponentStringInput> recipeTagPathInput;
    protected GuiComponentListInput<GuiComponentIngredientModifierInput> recipeModifiersInput;
    
	public GuiEditRecipeCraftingModifyNBT(GuiScreen parentScreen, String title, Addon addon, RecipeAddedCraftingModifyNBT recipe) {
		super(parentScreen, title);
		this.addon = addon;
		
		this.isNew = recipe == null;
		this.addition = this.isNew ? new RecipeAddedCraftingModifyNBT() : recipe;
	}
	...
	public void initComponents() {
		...
		this.recipeHasDifferentOutputInput = new GuiComponentBooleanInput(I18n.format("gui.edit.recipe.crafting.modify.differentOutput.label"), this) {
			...
			protected void setBoolean(boolean input) {
				...
				if (input) {
					GuiEditRecipeCraftingModifyNBT.this.recipeOutputInput.setHidden(false);
				} else {
					GuiEditRecipeCraftingModifyNBT.this.recipeOutputInput.setHidden(true);
				}
			}
		...
		this.recipeTagPathInput = new GuiComponentListInput<GuiComponentStringInput>(I18n.format("gui.edit.recipe.crafting.modifyNbt.nbtPath.label"), this) {
		...
		this.recipeTagPathInput.setRequired();
		
		for (String toAdd : this.addition.nbtPath) {
			GuiComponentStringInput input = this.recipeTagPathInput.createBlankComponent();
			input.setDefaultText(toAdd);
			this.recipeTagPathInput.addDefaultComponent(input);
		};
		...
		this.components.add(this.recipeTagPathInput);
	}
	...
	public void saveObject() {
		...
		String[] nbtPath = this.recipeTagPathInput.getComponents().stream().filter(component -> !component.getText().isEmpty()).map(GuiComponentStringInput::getText).collect(Collectors.toList()).toArray(new String[0]);
		
		if (nbtPath.length == 0) {
			this.mc.displayGuiScreen(new GuiMessageBox(this, I18n.format("gui.edit.recipe.problem.title"), new TextComponentTranslation("gui.edit.recipe.crafting.modifyNbt.problem.noNbtPath.message"), I18n.format("gui.buttons.back")));
			return;
		}
	...
	public void copyFrom(RecipeAddedCraftingModifyNBT addition) {
	...
	protected void copyFromOther() {
		...
		this.recipeTagPathInput.removeAllComponents();
		
		for (String toAdd : this.copyFrom.nbtPath) {
			GuiComponentStringInput input = this.recipeTagPathInput.createBlankComponent();
			input.setDefaultText(toAdd);
			this.recipeTagPathInput.addDefaultComponent(input);
		};
