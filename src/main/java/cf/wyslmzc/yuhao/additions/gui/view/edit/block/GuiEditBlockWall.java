...
/**
 * Page for adding a new wall block or editing an existing one.
 * 
 * @author Tmtravlr (Rebeca Rey)
 * @since April 2020
 */
public class GuiEditBlockWall extends GuiEditBlock<BlockAddedWall> {
    
	public GuiEditBlockWall(GuiScreen parentScreen, String title, Addon addon, BlockAddedWall block) {
		super(parentScreen, title, addon);
		
		this.isNew = block == null;
		
		if (this.isNew) {
			this.block = new BlockAddedWall();
	...
	protected GuiScreen getTextureDialogue(GuiScreen nextScreen) {
    	return new GuiEditBlockTexture(nextScreen, this.addon, this.block, this.isNew, BlockModelManager.BlockModelType.WALL);
    }
}
